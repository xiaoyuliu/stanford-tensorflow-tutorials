...
out = tf.case({tf.less(x, y): lambda: tf.add(x, y),
              ...
              default=lambda: tf.constant(0, tf.float32), exclusive=True)
print(sess.run(out))
...
x = tf.random_normal([300], stddev=1.0, mean=0.0)
y = tf.random_normal([300], stddev=1.0, mean=0.0)

sub = tf.subtract(x, y)
ave = tf.reduce_mean(sub, axis=0)
zeros = tf.zeros_like(ave)
out = tf.cond(tf.greater(zeros, ave),
